...
void IRAM_ATTR DSP_TYPE1::isr_clkHandler() {
  ...
  // If CS goes high, the packet is over
  if (cs) {
  ...
  // If we aren't in a transmission state, wait for start
  if (!this->_packet_transm_active) {
  ...
  // Read bit from Data Pin (MSB first)
  if (data) {
  ...
  // Every 8 bits, save the byte to the payload
...
uint8_t DSP_TYPE1::getPressedButton() {
  // Logic to determine which button is pressed based on the payload
  // This usually involves checking specific bytes in the DSP stream
  // that correspond to the front panel button matrix.
  uint8_t btn = 0;
  
  // Example mapping: Byte 6 is often the button status byte
